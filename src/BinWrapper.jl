...
"""
    low_level_exec(cmd::Cmd;
                   input::String = "",
                   tempfile::Bool = false)

Executes `cmd`. The `input` string is passed to the standard input of `cmd`. The `tempfile` parameter determines whether to use file-based (`true`) or memory-based (`false`) implementation. The function returns a tuple of command stdout (`out`), stderr (`err`) and return status (`ret`) of `cmd`.
...
Calls the `kbprog` binary with the input file determined by the `basepath` parameter. A number of output files with the same basepath, but different extensions, is generated by the `kbprog` binary.
...
Calls the `wordreduce` binary with the standard input generated based on the `words` array. Returns an array of reduced words. The `kbprog_call` function is called automatically if any of the input files does not exist.
